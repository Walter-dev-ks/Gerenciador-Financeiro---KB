    ...
    <AppShell>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 px-5 sm:grid-cols-3">
        <input
          ...
          className="col-span-2 h-9 min-w-0 rounded-lg border bg-card px-3 py-2 text-sm sm:col-span-1"
        />
            ...
            <li key={t.id} className="flex items-center gap-3 px-5 py-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-xs text-muted-foreground">
                  {person?.name ?? "-"} · {cat?.name ?? "Sem categoria"} ·{" "}
              ...
              <p
                className={`shrink-0 tabular-nums font-semibold ${isNegative ? "text-expense" : t.type === "INCOME" ? "text-income" : ""}`}
              >
                {isNegative ? "-" : t.type === "INCOME" ? "+" : ""}
                {brl(t.amount)}
              </p>
              <Button
                variant="ghost"
                size="icon"
                onClick={() => handleDelete(t.id, t.account_id, t.amount, t.type, t.is_paid)}
              >
