...
export function TransactionForm({ trigger }: { trigger?: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchPersons(), fetchAccounts(), fetchCategories()]).then(([p, a, c]) => {
      ...
      setAccounts(a);
      setCategories(c);
      setPersonId((current) => current || p[0]?.id || "");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (type !== "TRANSFER" && isPaid && !accountId) {
      return toast.error("Selecione a conta para atualizar o saldo disponivel");
    }
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Input
                ...
                max={60}
                value={parcels}
                onChange={(e) => setParcels(parseInt(e.target.value) || 1)}
              />
            </div>
          </div>

          <div className="flex items-center justify-between rounded-lg bg-muted p-3">
            <Label htmlFor="paid" className="!m-0">
